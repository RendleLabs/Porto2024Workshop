...
public class StreamSpanIntImpl
{
    ...
    public StreamSpanIntImpl(string filePath)
    {
        _filePath = filePath;
    }
    ...
    {
        var dict = new Dictionary<CityKey, IntAccumulator>();
    ...
    private static void ParseLine(ReadOnlySpan<byte> line, Dictionary<CityKey, IntAccumulator> dict)
    {
        // Porto;25.345
        int semicolon = line.IndexOf((byte)';');
        var city = line.Slice(0, semicolon);
        var key = CityKey.Create(city);
        var temperatureSpan = line.Slice(semicolon + 1);

        var temperature = ParseTemperature(temperatureSpan);

        if (!dict.TryGetValue(key, out var accumulator))
        {
            dict[key] = accumulator = new IntAccumulator(Encoding.UTF8.GetString(city));
        }
